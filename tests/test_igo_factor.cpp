...
TEST_F(TestIgoFactor1, Init) {
    ...
    for(int i = 0; i < 3; i++) {
        ASSERT_EQ(Lnz[i], Lnz_cor[i]);
    }
    int cor_idx = 0;
    for(int j = 0; j < 3; j++) {
        for(int idx = Lp[j]; idx < Lp[j] + Lnz[j]; idx++) {
            ASSERT_EQ(Li[idx], Li_cor[cor_idx]);
            ASSERT_FLOAT_EQ(Lx[idx], Lx_cor[cor_idx]);
            cor_idx++;
        }
    }
    for(int i = 0; i < 5; i++) {
...
class TestIgoFactor4 : public ::testing::Test {
public:
    igo_common* igo_cm = nullptr;
    igo_sparse* igo_A = nullptr;
    igo_sparse* igo_b = nullptr;
