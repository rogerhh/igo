...
PetscErrorCode DMCreateMatrix_ShellDA(DM dm, Mat *A)
{
  ...
  // cout << "In DMCreateMatrix_ShellDA, Before DMCreateMatrix, A size = " << M << N << endl;
  PetscCall(DMCreateMatrix(da, A));
  PetscCall(MatGetSize(*A, &M, &N));
  cout << "in dm create matrix " << endl;
  PetscCall(DMView(da, PETSC_VIEWER_STDOUT_SELF));
  cout << "In DMCreateMatrix_ShellDA, After DMCreateMatrix, A size = " << M << N << endl;
...
PetscErrorCode DMShellCreate_ShellDA(DM da, DM *dms)
{
  ...
  if (da) {
    PetscCall(DMShellCreate(PetscObjectComm((PetscObject)da), dms));
      cout << "in dm create shell da " << endl;
      PetscCall(DMView(da, PETSC_VIEWER_STDOUT_SELF));
...
PetscErrorCode ComputeMatrix_ShellDA(KSP ksp, Mat J, Mat jac, void *ctx)
{
  ...
  PetscCall(DMShellGetContext(dm, &da));
  cout << "In ComputeMatrix_ShellDA" << endl;
  PetscCall(MatView(J, PETSC_VIEWER_STDOUT_SELF));
  cout << "jac before = " << endl;
  PetscCall(MatView(jac, PETSC_VIEWER_STDOUT_SELF));
