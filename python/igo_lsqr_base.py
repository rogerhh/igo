...
class IgoLSQRBase(IgoBase):
    ...
    def incremental_opt(self, A_tilde, b_tilde, A_hat, b_hat, diagLamb, params):
        ...
        # 6. Use conjugate gradient to solve A^TAx = A^Tb
        linOps = applyPreconditionedA(AP, negAP, diagLambP, self.L)
        print("lsqr ", params["cg_tolerance"])
        x0 = np.zeros((AP.shape[1], ))
        LTxP, istop, itn, normr = scipy.sparse.linalg.lsqr(linOps, self.b.A.squeeze(), atol=float(params["cg_tolerance"]), btol=float(params["cg_tolerance"]), iter_lim=int(params["max_cg_iter"]))[:4]
        ...
        if params["step"] in params["lc_steps"] and params["outer_iter"] == params["profile_outer_iter"]:
            ...
            params["output_yaml_obj"][step]["lsqr_iter"] = itn
